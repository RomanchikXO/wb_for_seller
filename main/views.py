                ...
                items[row["nmid"]]["subitems"].append(
                    {
                        ...
                        "stock": row["total_quantity"],
                        "turnover": int(row["total_quantity"] / (row["total_orders"] / order_periods)) if row["total_orders"] else row["total_quantity"],
                        "rec_delivery": 0,
                    }
                )

        for key, value in items.items():
            items[key]["turnover_total"] = int(items[key]["stock"] / (items[key]["orders"] / order_periods)) \
